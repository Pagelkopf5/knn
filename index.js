...
NodeList.prototype.determineUnknown = function() {
    ...
    for (var i in this.nodes)
    {
        ...
        if ( ! this.nodes[i].type)
        {
            ...
            console.dir(this.nodes[i].guessType(this.k));
            

            var node = document.createElement("li");
            
            if(this.nodes[i].guessType(this.k).house != undefined){
                var test = document.createTextNode('house: '+ this.nodes[i].guessType(this.k).house + " ");
                node.appendChild(test);
            }
            if(this.nodes[i].guessType(this.k).flat != undefined){
                var test = document.createTextNode('flat: '+ this.nodes[i].guessType(this.k).flat + " ");
                node.appendChild(test);
            }
            if(this.nodes[i].guessType(this.k).apartment != undefined){
                var test = document.createTextNode('apartment: '+ this.nodes[i].guessType(this.k).apartment + " ");
                node.appendChild(test);
            }
...
var run = function() {
    var random_notNull = 1 + Math.round( Math.random() * 10 );

    nodes = new NodeList(random_notNull);
    for (var i in data)
    {
        nodes.add( new Node(data[i]) );
    }
    var random_rooms = random_notNull - 1;
